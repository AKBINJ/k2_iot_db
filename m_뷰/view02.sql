...
# 'v_view_test02', 'CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW 
# `v_view_test02` AS select `member`.`mem_id` AS `mem_id`,`member`.`mem_name` AS `mem_name` 
...
# > member 테이블로 생성한 v_memebr 뷰는 mem_id, mem_name
