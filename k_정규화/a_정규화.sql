...
/*
    ...
    5. 정규화의 단계
		...
		2) 제 2 정규형
			: 1NF 만족 + 키본키에 대한 완전 함수 종속
            - 부분 종속 제거 (복합기의 일부 컬럼에만 의존하는 속성 분리)
...
CREATE TABLE IF NOT EXISTS `student_course` (
    ...
    studnet_name varchar(50) not null,
    course_id bigint not null,
    course_name varchar(50) not null,
    course_instructor varchar(50) not null,
    primary key (student_id, course_id) # student_id + course_id는 중복 허용 / 비워질 수 없음
);
...
# cf) 베스트 프렉티스(best pratice)
# : 특정 문제를 해결하거나 목표를 달성함에 있어 가장 효과적으고 검증된 방법
