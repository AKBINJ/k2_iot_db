...
# Non_unique : 증복 가능 여부
# >> 0(False) : 중복 허용이 되지 않는 인데스 (유일한 값)
# >> 1(True) : 중복 가능한 인데스

# Key_name: PRIMSRY (기본 키 설정으로 자동 생성도니 인덱스)
# Column_name : 인덱스가 부여된 컬럼뎜

# Seq_in_index: 인덱스 안에서의 커럼 순서
# >> 복합키에서 인덱스로 사용하는 복합인덱스에서 두 개의 컬럼의 순서 (우선순위)
...
SELECT * FROM members; -- mem_id를 기준으로 오름차순 (ASF > BLK > IVE > LSF)
...
select * FROM members; -- 인덱스 저장 후 (PK 설정 후) 추가 데이터 삽입 시: 정렬 기준에 따라 자동 정렬

-- 정렬되지 않는 보조 인덱스 --
# : 고유 키 지정 시 생서 / 테이블 당 여러개 가능 / 정렬 X
