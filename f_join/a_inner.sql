...
# +) 해당 특정 회원의 정보까지 포함
SELECT P.purchase_id, M.member_id, M.name, M.area_code
...
# : 구매 이력이 있는 회원만 조회 
select
	distinct M.name, M.contact # 조인한 테이블의 테이터가 반드시 조회될 필요는 없음
from 
	`members` M
    join `purchases` P
    on M.member_id = P.member_id;
...
## 내부 조인 사용 시 주의점 ##
# 1) 동일 컬럼 이름 : 반드시 '컬럼.컬럼명' 형태로 구분하여 작성
...
# 구매 금액이 가장 높은 회원의
# member_id, name, total_amount(총 구매 금액)