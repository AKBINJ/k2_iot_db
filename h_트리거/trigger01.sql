...
-- 데이터 무결성: 수동으로 처리할 작업을 자동으로 처리 데이터 오류 방지 (회원 삭제 전 해당 회원 내역 백업)
...
# 2) 트랜잭션
# : 여러 작업이 하나의 작업 단위로 원자성 있게 '묶음'
# - 개발자가 직접 명시적으로 시작
# - COMMIT(적용 시점), ROLLBACK(취소)로 직접 제어
# > 데이터의 일관성 보장, 요구 복구
# EX) 주문 처리 중 오류 시 전체 작업 롤백(취소)

# cf) 원자성(Atomicitiy): 모두 성공 또는 모두 실패 (하나라도 살패 시 모두 롤백)